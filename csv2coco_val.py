...
if __name__ == '__main__':
    csv_file = "val.csv"
    image_dir = "/home/sda/videonet/val/image/val/"
    saved_coco_path = "./"
    # 整合csv格式标注文件
    ...
    for file in val_keys:
      if not os.path.exists('{}coco/images/val2014/{}'.format(saved_coco_path,file.split('/')[0])):
        print(file.split('/')[0])
        os.makedirs('{}coco/images/val2014/{}'.format(saved_coco_path,file.split('/')[0]))
        if not os.path.exists("{}coco/images/val2014/{}".format(saved_coco_path,file)):
          shutil.copy(image_dir+file,"{}coco/images/val2014/{}".format(saved_coco_path,file))
      elif os.path.exists('{}coco/images/val2014/{}'.format(saved_coco_path,file.split('/')[0])):
        if not os.path.exists("{}coco/images/val014/{}".format(saved_coco_path,file)):
          shutil.copy(image_dir+file,"{}coco/images/val2014/{}".format(saved_coco_path,file))
    l2c_train = Csv2CoCo(image_dir=image_dir,total_annos=total_csv_annotations)
    val_instance = l2c_train.to_coco(val_keys)
    l2c_train.save_coco_json(val_instance, '%scoco/annotations/instances_val2014.json'%saved_coco_path)
    #for file in val_keys:
